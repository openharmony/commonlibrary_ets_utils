...
public:
    ...
    static napi_value NewError(napi_env env, int32_t errCode, const char* errMessage)
    {
        ...
        } else if (errCode == ERR_CANCAL_NONEXIST_TASK) {
            errTitle = "The task does not exist when it is canceled, ";
        } else if (errCode == ERR_CANCAL_RUNNING_TASK) {
    ...
    static const int32_t ERR_WORKER_UNSUPPORTED = 10200005; // 10200005 : the invoked API is not supported in workers
    ...
    static const int32_t ERR_CANCAL_NONEXIST_TASK = 10200015; // 10200015 : the task does not exist when it is canceled
    static const int32_t ERR_CANCAL_RUNNING_TASK = 10200016; // 10200016 : the task is executing when it is canceled
};
} // namespace Commonlibrary::Concurrent::Common::Helper
