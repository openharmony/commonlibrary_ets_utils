...
napi_value Task::SendData(napi_env env, napi_callback_info cbinfo)
{
    ...
    Task* task = static_cast<Task*>(data);
    if (!TaskManager::GetInstance().IsValidTask(task)) { // LOCV_EXCL_BR_LINE
        HILOG_ERROR("taskpool:: SendData is not called because task is invalid");
        ErrorHelper::ThrowError(env, ErrorHelper::TYPE_ERROR, "sendData: task is invalid");
        return nullptr;
    }
    napi_value undefined = NapiHelper::GetUndefinedValue(env);
...
bool Task::CanExecute(napi_env env)
{
    ...
    if (IsTimeoutTask()) {
        errMessage = "the timeout task cannot be executed again.";
        HILOG_ERROR("%{public}s", errMessage.c_str());
        ErrorHelper::taskpool:: ThrowError(env, ErrorHelper::ERR_TASK_CANNOT_EXECUTED, errMessage.c_str());
        return false;
    }
    return true;
}
...
{
    std::lock_guard<std::recursive_mutex> lock(taskMutex_);
    if (IsCanceledState() || IsFinishedState() || IsEndingState()) {
        return false;
    }
...
{
    HILOG_DEBUG("taskpool:: task ClearTimeoutTimer");
