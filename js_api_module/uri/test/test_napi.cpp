...
HWTEST_F(NativeEngineTest, ModuleTest011, testing::ext::TestSize.Level0)
{
    napi_env env = (napi_env)engine_;
    napi_value exports = nullptr;
    napi_create_object(env, &exports);
    ...
    napi_value uriClass = nullptr;
    napi_value constructorArgs[1] =  { 0 };
    std::string input = "http://username:password@www.uritest.com:99/path/abc?query#fagment";
    napi_create_string_utf8(env, input.c_str(), input.size(), &constructorArgs[0]);
    napi_status status = napi_get_named_property(env, exports, "Uri", &uriClass);
    napi_value instance = nullptr;
    status = napi_new_instance(env, uriClass, 1, constructorArgs, &instance);
    napi_value tempFn = nullptr;
    napi_value result = nullptr;
    napi_value key = StrToNapiValue(env, "qqwwee");
    napi_value value = StrToNapiValue(env, "erfa");
    napi_value keyArgs[] = { key, value };
    napi_get_named_property(env, instance, "addQueryValue", &tempFn);
    napi_call_function(env, instance, tempFn, 2, keyArgs, &result);
    std::string res = GetStringUtf8(env, result);
    ASSERT_STREQ(res.c_str(), "http://username:password@www.uritest.com:99/path/abc?query&qqwwee=erfa#fagment");
    napi_value segment = StrToNapiValue(env, "wqaaas");
    napi_value segargs[] = { segment };
    napi_get_named_property(env, instance, "addSegment", &tempFn);
    napi_call_function(env, instance, tempFn, 1, segargs, &result);
    res = GetStringUtf8(env, result);
    ASSERT_STREQ(res.c_str(), "http://username:password@www.uritest.com:99/path/abc/wqaaas?query#fagment");
    value = StrToNapiValue(env, "https:");
    napi_set_named_property(env, instance, "scheme", value);
    napi_value uriProperty = nullptr;
    napi_get_named_property(env, instance, "scheme", &uriProperty);
    res = GetStringUtf8(env, uriProperty);
    ASSERT_STREQ(res.c_str(), "https");
    value = StrToNapiValue(env, "usernameinfo");
    napi_set_named_property(env, instance, "userInfo", value);
    uriProperty = nullptr;
    napi_get_named_property(env, instance, "userInfo", &uriProperty);
    res = GetStringUtf8(env, uriProperty);
    ASSERT_STREQ(res.c_str(), "usernameinfo");
}

HWTEST_F(NativeEngineTest, ModuleTest012, testing::ext::TestSize.Level0)
{
    napi_env env = (napi_env)engine_;
    napi_value exports = nullptr;
    napi_create_object(env, &exports);
    ...
    napi_value instance = nullptr;
    status = napi_new_instance(env, uriClass, 1, constructorArgs, &instance);
    napi_value value = StrToNapiValue(env, "/path1/path2");
    napi_set_named_property(env, instance, "path", value);
    napi_value uriProperty = nullptr;
    napi_get_named_property(env, instance, "path", &uriProperty);
    std::string res = GetStringUtf8(env, uriProperty);
    ASSERT_STREQ(res.c_str(), "/path1/path2");
    value = StrToNapiValue(env, "qwfagm");
    napi_set_named_property(env, instance, "fragment", value);
    uriProperty = nullptr;
    napi_get_named_property(env, instance, "fragment", &uriProperty);
    res = GetStringUtf8(env, uriProperty);
    ASSERT_STREQ(res.c_str(), "qwfagm");
    value = StrToNapiValue(env, "aaa=bbb");
    napi_set_named_property(env, instance, "query", value);
    uriProperty = nullptr;
    napi_get_named_property(env, instance, "query", &uriProperty);
    res = GetStringUtf8(env, uriProperty);
    ASSERT_STREQ(res.c_str(), "aaa=bbb");
    value = StrToNapiValue(env, "user@www.test112.cn:189");
    napi_set_named_property(env, instance, "authority", value);
    uriProperty = nullptr;
    napi_get_named_property(env, instance, "authority", &uriProperty);
    res = GetStringUtf8(env, uriProperty);
    ASSERT_STREQ(res.c_str(), "user@www.test112.cn:189");
    value = StrToNapiValue(env, "//uscffac@www.testut.cn:223?key=val");
    napi_set_named_property(env, instance, "ssp", value);
    uriProperty = nullptr;
    napi_get_named_property(env, instance, "ssp", &uriProperty);
    res = GetStringUtf8(env, uriProperty);
